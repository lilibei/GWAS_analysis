fst <- function(dataframe, colors=c("gray10", "gray50"), ymax="max", suggestiveline=5, genomewideline=8, annotate=NULL, ...) {
    ...
    if (numchroms==1) {
        with(d, plot(pos, logp, ylim=c(0,1), ylab=expression(-log[10](italic(p))), xlab=paste("Chromosome",unique(d$CHR),"position"), ...))
    }	else {
        with(d, plot(pos, logp, ylim=c(0,1), ylab='Fst', xlab="Chromosome", xaxt="n", type="n", ...))
        axis(1, at=ticks, lab=chrnames, ...)
        icol=1
        for (i in 1:numchroms) {
            with(d[d$CHR==chrnames[i], ],points(pos, logp, pch=16,cex=0.5,col=colors[icol], ...))
            icol=icol+1
    	}
    }
